...
def read_tfrecords(filename,sess):
    ...
    traindata=tf.reshape(features['data'],(20,20))
    print ('jjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjjj')
    #with tf.Session() as sess:
    traindata=traindata.eval(sess=session)
    print (traindata.shape)
    #traindata=traindata.eval()
    traindata=np.pad(traindata,(102,102),(102,102))
    traindata= tf.constant(traindata)
    traindata=tf.reshape(traindata,(224,224,1))
    print (traindata.shape)
    labeldata =features['label']
    return traindata,labeldata

def get_batch_images(tffilename,sess):
  with tf.device('/cpu:0'):
    train_set = os.path.join(folder_of_dataset,tffilename)
    #test_set = os.path.join(folder_of_dataset,'outputtest.tfrecords')
    train_data,train_label = read_tfrecords(train_set,sess)
    capacity = 1000+3*batch_size
    train_data,train_label = tf.train.shuffle_batch([train_data,train_label],batch_size = batch_size,capacity = capacity,min_after_dequeue = 30)
    #train_data = np.pad(train_data,(102,102),(102,102))
...
def step_train(train_op,loss,accuracy,train_record_file,val_record_file,
               train_nums,train_log_step,
               val_nums,val_log_step,
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_steps + 1):
            train_images_batch,train_labels_batch = get_batch_images(train_record_file,sess)
            val_images_batch,val_labels_batch = get_batch_images(val_record_file,sess)
...
if __name__ == '__main__':
    train_record_file='140*400train.tfrecords'
    val_record_file='140*400test.tfrecords'
    ...
    snapshot_prefix='inception_models/model.ckpt'
