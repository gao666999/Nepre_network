...
def read_tfrecords(filename):
    ...
    train_data=tf.reshape(features['data'],(140,400,1))
...
def train(train_record_file,
    ...
    #print (train_images_batch.shape,train_labels_batch)
    # Define the model:
    ...
    print(out.shape,input_labels.shape)
    # Specify the loss function: tf.losses定义的loss函数都会自动添加到loss函数,不需要add_loss()了
    w_fc2 = tf.Variable(tf.truncated_normal([64,2],stddev=0.1))
    b_fc2 = tf.Variable(tf.constant(0.1,shape=[2]))
    #fc_weights= tf.get_variable('weight',[2,2],initializer)
    out = tf.matmul(out,w_fc2)+b_fc2
    #y_conv=tf.nn.softmax(tf.matmul(out,w_fc2)+b_fc2)
    #cross_entropy=tf.reduce_mean(-tf.reduce_sum(y_conv*tf.log(y_conv),reduction_indices=[1]))
...
if __name__ == '__main__':
    train_record_file='140*400pisanewtrain.tfrecords'
    val_record_file='140*400pisanewtest.tfrecords'
    ...
    snapshot_prefix='inception_models_140/model.ckpt'
