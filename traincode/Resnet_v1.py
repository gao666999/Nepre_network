...
labels_nums = 2  # 类别个数
batch_size = 64
resize_height = 20  # mobilenet_v1.default_image_size 指定存储图片高度
resize_width = 20   # mobilenet_v1.default_image_size 指定存储图片宽度
depths = 1
...
def read_tfrecords(TFpath,is_train=True):
    TFfiles=tf.train.match_filenames_once(TFpath)
    filename_quene=tf.train.string_input_producer(TFfiles,shuffle=is_train)
    ...
    features=tf.parse_single_example(serialized_example,features={'label': tf.FixedLenFeature([2], tf.float32),
                                      'data' : tf.FixedLenFeature([400], tf.float32)
                                      })
    train_data=tf.reshape(features['data'],(20,20,1))
    label_data =features['label']
    return train_data,label_data

def get_batch_images(TFpath,is_train=True):
  with tf.device('/cpu:0'):
    #train_set = os.path.join(folder_of_dataset,tffilename)
    #test_set = os.path.join(folder_of_dataset,'outputtest.tfrecords')
    train_data,train_label = read_tfrecords(TFpath,is_train)
    capacity = 1000+3*batch_size
    train_data,train_label = tf.train.shuffle_batch([train_data,train_label],batch_size = batch_size,capacity = capacity,min_after_dequeue = 2*batch_size)
    return train_data,train_label

def net_evaluation(sess,loss,accuracy,val_images_batch,val_labels_batch):
...
def step_train(train_op,loss,accuracy,
    ...
    with tf.Session() as sess:
        ...
        for i in range(max_steps + 1):
            ...
            # val测试(测试全部val数据)
            if i % val_log_step == 0:
                mean_loss, mean_acc = net_evaluation(sess, loss, accuracy, val_images_batch, val_labels_batch)
...
def train(train_record_file,
    ...
    train_images_batch,train_labels_batch = get_batch_images(train_record_file)
    val_images_batch, val_labels_batch = get_batch_images(val_record_file,is_train=False)
    # Define the model:
...
if __name__ == '__main__':
    train_record_file='/home/xg666/Desktop/zdocktrain/usefulTFdata/20TF/train/*.tfrecords'
    val_record_file='/home/xg666/Desktop/zdocktrain/usefulTFdata/20TF/test/*.tfrecords'

    train_log_step=100
    base_lr = 0.001
    # 学习率
    # 重头开始训练的话，mobilenet收敛慢的一比，大概20000次迭代后，准确率开始蹭蹭的往上长,迭代十万次后准确率才70%
    max_steps = 1000000  # 迭代次数
    train_param=[base_lr,max_steps]

    val_log_step=500
    snapshot=2000#保存文件间隔
    snapshot_prefix='/home/xg666/Desktop/zdocktrain/savemodels/resnet_20/model.ckpt'
